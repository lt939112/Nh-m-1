  ...
  grid.innerHTML = list
    ...
    <div class="shoe-card">
      ...
      <button class="add-cart-btn" onclick="addToCart(${s.id})">Thêm vào giỏ</button>
        <button class="add-cart-btn" onclick="buyNow(${s.id})">Mua Ngay</button>
    </div>
  `,
    )
    .join("");
    ...
    <div class="detail-wrapper">
      ...
      <div class="detail-info">
        ...
        <button class="add-cart-btn" onclick="addToCart(${product.id})">THÊM VÀO GIỎ NGAY</button>
         <button class="add-cart-btn" onclick="buyNow(${product.id})">Mua Ngay</button>
      </div>
    </div>
  `;
}
...
// --- LOGIC GIỎ HÀNG ---
function addToCart(id, silent = false) { // Thêm tham số silent ở đây
...
  localStorage.setItem("cart", JSON.stringify(cart));
  ...
  // Chỉ hiện alert nếu KHÔNG PHẢI chế độ im lặng (silent)
  if (!silent) {
    alert(`Đã thêm "${product.name}" vào giỏ hàng!`);
  }
}
// --- CHỨC NĂNG MUA NGAY ---
// --- CHỨC NĂNG MUA NGAY (LƯU THẲNG VÀO LỊCH SỬ) ---
// --- CHỨC NĂNG MUA NGAY DÙNG CHUNG CHO MỌI TRANG ---
window.buyNow = (id) => {
  // 1. Kiểm tra đăng nhập trước khi cho mua
  const userLogin = localStorage.getItem("userLogin");
  if (!userLogin) {
    alert("Vui lòng đăng nhập để mua hàng nhanh!");
    window.location.href = "login.html";
    return;
  }

  // 2. Tìm thông tin đôi giày dựa trên ID (biến shoes đã có sẵn ở đầu file script.js)
  const product = shoes.find((s) => s.id === id);
  
  if (product) {
    // 3. Lấy danh sách lịch sử cũ từ máy (orders)
    let orders = JSON.parse(localStorage.getItem("orders")) || [];

    // 4. Tạo đối tượng đơn hàng mới (Mua 1 đôi duy nhất)
    const newOrder = {
      orderId: "DH" + Math.floor(Math.random() * 90000 + 10000),
      date: new Date().toLocaleString("vi-VN"),
      items: [{ ...product, qty: 1 }], // Đóng gói sp vào mảng
      total: product.price
    };

    // 5. Thêm vào lịch sử (Đẩy lên đầu mảng)
    orders.unshift(newOrder);
    localStorage.setItem("orders", JSON.stringify(orders));

    // 6. Thông báo cho người dùng biết đã xong
    alert(`MUA THÀNH CÔNG!\nSản phẩm: ${product.name}\nMã đơn: ${newOrder.orderId}\nBạn có thể kiểm tra trong mục Lịch sử.`);
    
    // Nếu các bạn muốn sau khi mua xong trang web tự load lại để cập nhật thì dùng:
    // location.reload(); 
  } else {
